...
try:
    map_path = Path("../maps/default.py")
    if map_path.is_file():
        with open(map_path) as _:
            settings.init_map(ast.literal_eval(_.read()))
    else:
        print("the path to the map file does not exist")
        exit(1)
except Exception:
...
class DRLRobot:
    ...
    def train(self, batch_size=32, epochs=1, verbose=0, model_history_output_path=".", save_history=False, callbacks=[]):
        if not self.exploit and len(self.memory) >= self.mini_batch_size:
            ...
            history = self.model.fit(self.state_array, target_f, batch_size=batch_size, epochs=epochs,
                                     verbose=verbose, callbacks=callbacks)

            if save_history:
                # graph history
                history_timestamp = datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S")
                fig = plt.figure()
                ax1 = fig.add_subplot(1, 1, 1)
                ax1.plot(history.history['loss'], color='b', label="Training Loss")
                ax1.legend()
                plt.savefig(os.path.join(model_history_output_path, f'{history_timestamp}.png'))
                plt.draw()
                plt.close()
                # save info for later
                if history is not None:
                    try:
                        hist_name = f'{history_timestamp}.pkl'
                        with open(os.path.join(model_history_output_path, hist_name), "wb") as f:
