...
from tensorflow_core.python import Zeros
from tensorflow_core.python.keras.initializers import glorot_normal
from tensorflow_core.python.keras.layers import Conv2D, MaxPooling2D, Dropout
...
def get_preprocess(output_dir='.'):
    spec = importlib.util.spec_from_file_location('preprocess', os.path.join(output_dir, 'squashpol_small.py'))
...
def preprocess(x, target_size=(10, 10)):
    ...
    tf.image.adjust_brightness(x, delta=0.2).numpy()
    #tf.image.adjust_contrast(x, 1).numpy()
    #tf.image.adjust_gamma(x, 0.5).numpy()
    tf.image.flip_left_right(x).numpy()
...
def build_fn(input_shape, optimizer, loss, output_activation, metrics):
    ...
    # Fully connected linear model
    model = Sequential()
    model.add(Conv2D(8, kernel_size=(3, 3), activation='relu', input_shape=(10, 10, 3), kernel_initializer=w_init,
                     bias_initializer=b_init))
    model.add(Conv2D(16, kernel_size=(3, 3), activation='relu', kernel_initializer=w_init, bias_initializer=b_init))
    model.add(Conv2D(32, kernel_size=(3, 3), activation='relu', kernel_initializer=w_init, bias_initializer=b_init))
    ...
    model.add(Dropout(0.25))
    model.add(Flatten())
    model.add(Dense(128, activation='relu', kernel_initializer=w_init, bias_initializer=b_init))
    # model.add(BatchNormalization())
    model.add(Dropout(0.25))
    model.add(Flatten())
    model.add(Dense(256, activation='relu', kernel_initializer=w_init, bias_initializer=b_init))
    # model.add(BatchNormalization())
    model.add(Dropout(0.25))
    model.add(Flatten())
    model.add(Dense(512, activation='relu', kernel_initializer=w_init, bias_initializer=b_init))
    # model.add(BatchNormalization())
    model.add(Dropout(0.25))
    model.add(Flatten())
...
def main():
    ...
    """
    # paths
    data_dir = '/Users/coltonbailey/Desktop/Fall2021/AI/squash/media/data/squashpol_small'
    ...
    for params in itertools.product(*param_grid_list):
        ...
        shutil.copyfile(__file__, os.path.join(output_path, 'squashpol_small.py'))
