...
def build_fn(input_shape, optimizer, learning_rate, loss, output_activation, metrics, neurons, dropout):
    ...
    model.add(
        Conv2D(128, kernel_size=(3, 3), activation='relu', kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg)))
    model.add(MaxPooling2D(2, 2))
    model.add(BatchNormalization())

    model.add(
        Conv2D(256, kernel_size=(3, 3), activation='relu', kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg)))
    model.add(MaxPooling2D(2, 2))
    model.add(BatchNormalization())

    model.add(
        Conv2D(512, kernel_size=(3, 3), activation='relu', kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg)))
