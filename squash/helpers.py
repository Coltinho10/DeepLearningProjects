...
def preprocess(x, target_size=(10, 10)):
    ...
    x = tf.image.central_crop(x, 0.8) #change target_size
...
def build_fn(input_shape, optimizer, learning_rate, loss, output_activation, metrics, neurons, dropout):
    ...
    model.add(Conv2D(128, kernel_size=(3, 3), activation='relu',
                     input_shape=input_shape, kernel_regularizer=tensorflow.keras.regularizers.l2(0.01)))
    model.add(MaxPooling2D(2, 2))
    model.add(BatchNormalization())

    model.add(Conv2D(128, kernel_size=(3, 3), activation='relu', kernel_regularizer=tensorflow.keras.regularizers.l2(0.01)))
    model.add(MaxPooling2D(2, 2))
    model.add(BatchNormalization())

    model.add(Conv2D(256, kernel_size=(3, 3), activation='relu', kernel_regularizer=tensorflow.keras.regularizers.l2(0.01)))
    model.add(MaxPooling2D(2, 2))
    model.add(BatchNormalization())
...
def main():
    ...
    param_grids = {
        ...
        'preprocess_params': ParameterGrid({
            'target_size': [(60, 60), (60, 60)],  # (192, 192)
        }),
